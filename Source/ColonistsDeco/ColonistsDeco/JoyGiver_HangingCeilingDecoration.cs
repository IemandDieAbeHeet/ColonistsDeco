...
namespace ColonistsDeco
{
    class JoyGiver_HangingCeilingDecoration : JoyGiver
    {
        public override Job TryGiveJob(Pawn pawn)
        {
            ...
            if (pawn.WorkTypeIsDisabled(WorkTypeDefOf.Construction) || pawn.IsPrisoner || pawn.ownership.OwnedBed == null || !pawn.TryGetComp<CompPawnDeco>().CanDecorate || pawn.story.traits.HasTrait(DefDatabase<TraitDef>.GetNamed("Ascetic")) || !Utility.TechLevelHasDecos(pawn.Faction.def.techLevel, DecoLocationType.Ceiling))
            {
                return null;
            }
